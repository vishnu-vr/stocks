...
def main_driver(ticker, percentage, start_date, end_date):
    ...
    for i in range(len(data)):
        ...
        # Define conditions
        conditions = [
            row['RSI'] < 60,  # Condition 1: RSI is below 60
            row['MACD'] > row['Signal_Line'],  # Condition 2: MACD line is above signal line
            row['MACD'] < 0,  # Condition 3: MACD line is under the histogram
            row['Close'] > row['EMA_44'],  # Condition 4: Above 44 EMA
            row['SMA_10'] > row['SMA_20']  # Condition 5: 10 SMA greater than 20 SMA
        ]

        # Calculate number of conditions to meet including MACD
        ...
        if macd_condition_met and conditions_met_count >= num_conditions_to_meet:
            # Set buy price and propagate
            buy_price = row['Close']
            data.iloc[i:, data.columns.get_loc('Buy_Price')] = buy_price

    # Calculate sell price based on a 5% increase
    data['Sell_Price'] = data['Buy_Price'] * 1.05

    # Check if the close price reaches the sell price
    data['Sell_Triggered'] = data['Close'] >= data['Sell_Price']

    # Check if there are any buy and sell points
    if data['Buy_Price'].notna().any() and data['Sell_Price'].notna().any():
        # Save to Excel if there are buy and sell points
        data.to_excel(f'delete_me\\{ticker}_backtest.xlsx', index=True)
        print(f"Backtest completed for {ticker}. Data saved to Excel.")
    else:
        print(f"No buy or sell points found for {ticker}. Skipping.")
...
if __name__ == "__main__":
    ...
    for ticker in nifty_stocks:
        ...
        try:
            main_driver(ticker, percentage, start_date, end_date)
